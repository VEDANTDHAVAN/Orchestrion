...
export const BorderLoadingIndicator = ({
  ...
  return (
    <>
      <div className="absolute -top-[2px] -left-[3px] h-[calc(100%+5px)] w-[calc(100%+5px)]">
        <style>
          {`
        @keyframes spin {
      ...
      `}
        ...
        <div className={cn("absolute inset-0 overflow-hidden rounded-[9px]", className,)}>
          <div className="spinner rounded-sm bg-[conic-gradient(from_0deg_at_50%_50%,rgb(42,67,233,0.5)_0deg,rgba(42,138,246,0)_360deg)]" />
        </div>
      </div>
      {children}
    </>
...
const StatusBorder = ({
  ...
  return (
    <>
      <div
        className={cn(
          "absolute -top-[2px] -left-[2.5px] h-[calc(100%+4px)] w-[calc(100%+4px)] rounded-md border-3",
          className,
        )}
      />
      {children}
